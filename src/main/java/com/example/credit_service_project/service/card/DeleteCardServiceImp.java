...
public class DeleteCardServiceImp implements CardService<DeleteCardDTOResponse, DeleteCardDTORequest> {
    ...
    public DeleteCardDTOResponse execute(DeleteCardDTORequest request) {
        var card = repository.findByIdAndCardNumber(request.getId(), request.getCardNumber());
        return card.map(c -> {
            var response = new DeleteCardDTOResponse(c.getId(), c.getCardNumber(),c.getAccount().getAccountNumber());
            repository.delete(c);
            return response;
